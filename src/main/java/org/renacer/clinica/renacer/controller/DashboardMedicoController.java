...
public class DashboardMedicoController implements Initializable {
    ...
    private final ObservableList<CitaAgendaResponse> agenda = FXCollections.observableArrayList();
    private final ObservableList<CitaAgendaResponse> pendientes = FXCollections.observableArrayList();
    ...
    private static final Medico TODOS = new Medico("0", "Todos", "los médicos", "—", "—");
            ...
            protected void updateItem(CitaAgendaResponse c, boolean empty) {
                ...
                switch (c.getEstado()) {
                    case EstadoCita.EN_CURSO:
                        setStyle("-fx-background-color: #fff3cd;");   // amarillo
                        break;
        ...
        lstPendientes.setCellFactory(lv -> new ListCell<CitaAgendaResponse>() {
            ...
            protected void updateItem(CitaAgendaResponse c, boolean empty) {
                ...
                if (empty || c == null) {
                    ...
                    setText(c.getFecha() + "  " + c.getHora() + "   " + c.getPaciente()
                            + "\n   " + c.getMedico() + " · " + c.getEspecialidad()
                            + "  [" + c.getEstado() + "]");
                }
            }
        });
 
        // Al hacer clic en una pendiente, salta a ese día y la selecciona en la tabla
        lstPendientes.getSelectionModel().selectedItemProperty().addListener((o, ant, act) -> {
            if (act == null) return;
    ...
    private void configurarCombos() {
        cboMedico.setConverter(new StringConverter<Medico>() {
            ...
            public String toString(Medico m) {
                if (m == null) return "";
                if ("0".equals(m.getIdMedico())) return "Todos los médicos";
                return m.getNombres() + " " + m.getApellidos() + " (" + m.getEspecialidad() + ")";
            }
            @Override
            public Medico fromString(String s) { return null; }
        });
 
        cboPacienteNuevo.setConverter(new StringConverter<Paciente>() {
            @Override
        ...
        cboMedico.valueProperty().addListener((o, a, b) -> refrescarTodo());
        dpFecha.valueProperty().addListener((o, a, b) -> refrescarTodo());
    ...
    private void marcarDiasConCitas() {
        dpFecha.setDayCellFactory(picker -> new DateCell() {
            ...
            public void updateItem(LocalDate fecha, boolean empty) {
                ...
                setTooltip(null);
                setStyle("");
                if (empty || fecha == null) return;
 
                Integer total = conteoMes.get(fecha);
                if (total != null && total > 0) {
                    setStyle("-fx-background-color: #cfe2ff; -fx-font-weight: bold;");
                    setTooltip(new Tooltip(total + " cita(s)"));
                }
            }
        });
    }
    ...
    private void cargarMedicos() {
        try {
            ObservableList<Medico> lista = FXCollections.observableArrayList(TODOS);
            lista.addAll(new main.java.org.renacer.clinica.renacer.service.MedicoService().listar());
            cboMedico.setItems(lista);
            cboMedico.setValue(TODOS);
        } catch (SQLException ex) {
            Alertas.error("No se pudieron cargar los médicos: " + ex.getMessage());
        }
    }
 
    private void cargarPacientes() {
        try {
            cboPacienteNuevo.setItems(FXCollections.observableArrayList(pacienteRepository.listar()));
        } catch (SQLException ex) {
    ...
    private void cargarConteoMes() {
        try {
            ...
            conteoMes = citaService.conteoPorDia(base, idMedicoFiltro());
            marcarDiasConCitas();   // fuerza el repintado de las celdas
        } catch (SQLException ex) {
    ...
    private void cargarAgenda() {
        try {
            LocalDate dia = dpFecha.getValue() == null ? LocalDate.now() : dpFecha.getValue();
            List<CitaAgendaResponse> datos = citaService.agendaPorDia(dia, idMedicoFiltro());
            agenda.setAll(datos);
 
            long activas = datos.stream()
                    .filter(c -> EstadoCita.ocupaAgenda(c.getEstado()))
                    .count();
            lblResumen.setText("Citas del " + dia + ": " + activas + " activa(s) de " + datos.size() + " registrada(s).");
 
        } catch (SQLException ex) {
            Alertas.error("No se pudo cargar la agenda: " + ex.getMessage());
        }
    }
 
    private void cargarPendientes() {
        try {
            pendientes.setAll(citaService.pendientes(idMedicoFiltro()));
        } catch (SQLException ex) {
            Alertas.error("No se pudieron cargar las citas pendientes: " + ex.getMessage());
        }
    }
    ...
    private void cargarHorasLibres() {
        try {
            ...
            if ("0".equals(idMedico) || dpFecha.getValue() == null) {
                cboHoraLibre.getItems().clear();
                return;
            }
            List<LocalTime> libres = citaService.horariosDisponibles(idMedico, dpFecha.getValue());
            cboHoraLibre.setItems(FXCollections.observableArrayList(libres));
            if (!libres.isEmpty()) cboHoraLibre.setValue(libres.get(0));
        } catch (SQLException ex) {
            Alertas.error("No se pudo calcular la disponibilidad: " + ex.getMessage());
        }
    }
    ...
    private void mostrarSeleccion(CitaAgendaResponse c) {
        if (c == null) {
            lblCitaSeleccionada.setText("Ninguna cita seleccionada");
        ...
        lblCitaSeleccionada.setText(c.getFecha() + " " + c.getHora() + " · " + c.getPaciente()
                + "  |  " + c.getMedico() + " (" + c.getEspecialidad() + ")  ·  " + c.getEstado());
 
        String estado = c.getEstado();
        btnIniciar.setDisable(!EstadoCita.PROGRAMADA.equals(estado));
        btnFinalizar.setDisable(!EstadoCita.EN_CURSO.equals(estado));
        btnCancelar.setDisable(EstadoCita.FINALIZADA.equals(estado) || EstadoCita.CANCELADA.equals(estado));
        btnReceta.setDisable(!c.tieneConsulta());
    }
    ...
    private void onIniciarConsulta() {
        CitaAgendaResponse c = tblAgenda.getSelectionModel().getSelectedItem();
        try {
            citaService.iniciarConsulta(c);
            Alertas.info("La cita pasó a estado 'en curso'.");
            refrescarTodo();
        } catch (IllegalArgumentException ex) {
            Alertas.advertencia(ex.getMessage());
        } catch (SQLException ex) {
            Alertas.error("Error de base de datos: " + ex.getMessage());
        }
    }
    ...
    private void onCancelarCita() {
        CitaAgendaResponse c = tblAgenda.getSelectionModel().getSelectedItem();
        if (c == null) {
            Alertas.advertencia("Seleccione una cita de la tabla.");
            return;
        }
        if (!Alertas.confirmar("¿Cancelar la cita de " + c.getPaciente() + " del " + c.getFecha() + " a las " + c.getHora() + "?")) {
            return;
        }
        try {
    ...
    private void onAgregarReceta() {
        CitaAgendaResponse c = tblAgenda.getSelectionModel().getSelectedItem();
        try {
            citaService.agregarReceta(c, txtMedicamentos.getText(), txtIndicaciones.getText());
            Alertas.info("Receta agregada a la consulta.");
